...
include dirname(__FILE__) . "/validators/catPezValidator.php";
  ...
  switch($_GET["command"]){
    case "read"://---------------------------------------------------------------------------------------------------READ
      if(isset($_GET["userId"]) && isset($_GET["verif"])){
        ...
        if($validation){
          $result = $conn->prepare('SELECT nombre_criatura FROM catpeces WHERE usuario_id = ?');
    ...
    case "create"://---------------------------------------------------------------------------------------------------CREATE
      if(isset($_GET["userId"]) && isset($_GET["verif"])){
        ...
        if(isset($postdata) && !empty($postdata)){
          ...
          if($validation){
            $result = $conn->prepare('INSERT INTO catpeces(usuario_id, nombre_criatura) VALUES (?, ?)');
    ...
    case "delete"://---------------------------------------------------------------------------------------------------DELETE
      if(isset($_GET["userId"]) && isset($_GET["verif"]) && isset($_GET["nombreCriatura"])){
        ...
        if($validation){
          $result = $conn->prepare('DELETE FROM catpeces WHERE usuario_id = ? AND nombre_criatura = ?');
          $result->bind_param('is',$userId, $nombreCriatura);
          $result->execute();
        }else{
          print("No se cumplen los requisitos");
